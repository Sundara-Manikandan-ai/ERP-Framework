...
const getPageData = createServerFn({ method: 'GET' })
  .middleware([resourceMiddleware('factories')])
  .handler(async ({ context }) => {
    const factories = await db.factory.findMany({
      orderBy: { createdAt: 'desc' },
      include: { _count: { select: { transactions: true } } },
    })
    return { factories, access: extractAccess(context) }
  })
...
const createFactory = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    await db.factory.create({ data: { name: data.name, address: data.address ?? null } })
    return { success: true }
  })
...
const deleteFactory = createServerFn({ method: 'POST' })
  .middleware([adminMiddleware])
  ...
  .handler(async ({ data }) => {
    ...
    if (transactionCount > 0)
      throw new Error(`Cannot delete — ${transactionCount} transaction(s) reference this factory.`)
    if (batchCount > 0)
      throw new Error(`Cannot delete — ${batchCount} upload batch(es) reference this factory.`)
...
function FactoriesPage() {
  const { factories, access } = Route.useLoaderData()
            ...
            <RoleGate {...access} requireAdmin>
              <div className="flex items-center justify-end gap-1">
                ...
                <DeleteDialog
                  ...
                  description={<>Are you sure you want to delete <strong>{factory.name}</strong>? This cannot be undone.</>}
                  disabled={factory.transactionCount > 0}
                  disabledReason={factory.transactionCount > 0 ? `This factory has ${factory.transactionCount} transaction(s) and cannot be deleted.` : undefined}
                  onConfirm={async () => { await deleteFactory({ data: { id: factory.id } }); refresh() }}
                />
              </div>
            </RoleGate>
          )
